...
| Mobile application | Keep pending role review | Dedicated operational workflows exist for tasks, calendar, check activity, cash, unit details, and inventory. Mobile V2 now has an explicitly Admin-only initial rollout, an admin reservations calendar, and a daily cleanings page with city filtering, check-in/check-out activity, all cleaning types (including standalone services), cleaning status, assignment visibility, and unit/condo access details. The underlying completion policy preserves Cleaner self-completion and Admin/Manager authority, but manager/cleaner Mobile V2 access and data scope must be tested and approved before the rollout guard is changed. |
