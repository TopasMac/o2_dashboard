...
| Mobile application | Keep pending role review | Dedicated operational workflows exist for tasks, calendar, check activity, cash, unit details, and inventory. Mobile V2 has an admin reservations calendar and a daily cleanings page with city filtering, check-in/check-out activity, all cleaning types (including standalone services), cleaning status, assignment visibility, and unit/condo access details. The feature branch now includes a controlled Cleaner pilot: Cleaners receive only the cleanings page, their city scope is enforced by the backend, and self-only completion remains protected. Production remains Admin-only until the Cleaner pilot is released and verified with an approved real account; Manager access is still disabled pending separate testing. |
