...
class HKMonthlySummaryService
{
    ...
    /**
     * Return HR ledger rows (salary + advance) for HK cost centres for a month.
     *
     * Output columns:
     *  - id
     *  - employee_id
     ...
     *  - area
     */
    public function getHrByMonth(int $year, int $month): array
    {
        if ($month < 1 || $month > 12) {
        ...
        $sql = <<<SQL
            ...
            WHERE l.`type` IN ('salary', 'advance')
